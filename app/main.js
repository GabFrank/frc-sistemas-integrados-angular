...
if (process.platform === 'linux') {
    const origDoInstall = electron_updater_1.autoUpdater.doInstall;
    if (origDoInstall) {
        electron_updater_1.autoUpdater.doInstall = function (options) {
            ...
            if (appImageFile && fs.existsSync(appImageFile)) {
                ...
                try {
                    ...
                    const env = Object.assign(Object.assign({}, process.env), { APPIMAGE_SILENT_INSTALL: 'true' });
...
ipcMain.on('check-for-update-manual', () => {
    ...
    electron_updater_1.autoUpdater.checkForUpdates().then(result => {
        ...
        if (win)
            win.webContents.send('update-status', `Error: ${err === null || err === void 0 ? void 0 : err.message}`);
    });
});
 ...
 */
function correrPowerShell(script, timeoutMs = 20000) {
    return new Promise((resolve) => {
        const encoded = buffer_1.Buffer.from(script, 'utf16le').toString('base64');
        const proc = (0, child_process_1.spawn)('powershell.exe', ['-NoProfile', '-NonInteractive', '-EncodedCommand', encoded]);
        let stdout = '';
        let stderr = '';
        let terminado = false;
        const finish = (r) => {
 ...
 */
function detectarDispositivosWindows() {
    return __awaiter(this, void 0, void 0, function* () {
        const lista = [];
        const puertosConCola = new Set();
        ...
        const psColas = "$ErrorActionPreference='SilentlyContinue';"
        ...
        const colas = yield correrPowerShell(psColas);
        filasDeJson(colas.stdout).forEach((c) => {
        ...
        const psPuertos = "$ErrorActionPreference='SilentlyContinue';"
            + "Get-PrinterPort | Where-Object { $_.Name -like 'USB*' } | Select-Object Name | ConvertTo-Json -Compress";
        const puertos = yield correrPowerShell(psPuertos);
        const libres = [];
        filasDeJson(puertos.stdout).forEach((pp) => {
            const nombre = String(pp && pp.Name ? pp.Name : '').trim();
            if (!nombre || puertosConCola.has(nombre.toUpperCase())) {
                return;
            }
            libres.push(nombre);
            lista.push({
        ...
        const psPnp = "$ErrorActionPreference='SilentlyContinue';"
        ...
        const pnp = yield correrPowerShell(psPnp);
        filasDeJson(pnp.stdout).forEach((d) => {
...
function detectarDispositivosLinux() {
    return new Promise((resolve) => {
        const proc = (0, child_process_1.spawn)('lpinfo', ['-v']);
        let stdout = '';
        proc.stdout.on('data', (d) => { stdout += d.toString(); });
        ...
        proc.on('close', () => {
            ...
            stdout.split(/\r?\n/).forEach((linea) => {
                ...
                if (uri.startsWith('usb://')) {
                    ...
                    try {
                        nombre = decodeURIComponent(resto.replace(/\//g, ' ')).trim();
                    }
                    catch (_a) {
                        nombre = resto;
                    }
...
function detectarDispositivosLocales() {
    return __awaiter(this, void 0, void 0, function* () {
 ...
 */
function instalarImpresoraWindowsUsb(cola, puerto) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const r = yield correrPowerShell(ps, 40000);
 ...
 */
function imprimirRawWindows(cola, buffer) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const r = yield correrPowerShell(ps, 30000);
        try {
            fs.unlinkSync(tmp);
        }
        catch ( /* noop */_a) { /* noop */ }
 ...
 */
function compartirImpresoraWindows(queue, centralIp) {
    return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (process.platform === 'win32') {
                    ...
                    if (uri.toLowerCase().startsWith(ESQUEMA_USB_WIN)) {
                        resolve(yield instalarImpresoraWindowsUsb(colaSegura, uri.substring(ESQUEMA_USB_WIN.length)));
                        return;
                    }
...
function etiquetaPuerto(puerto) {
    if (puerto === 9100)
        return 'raw:9100';
    if (puerto === 631)
        return 'ipp';
...
function obtenerOCrearEntrada(mapa, ip) {
    let e = mapa.get(ip);
    if (!e) {
...
function descubrirImpresorasMdns(timeoutMs, locales) {
    return new Promise((resolve) => {
        const mapa = new Map();
        let bonjour = null;
        try {
        ...
        const primeraIpv4 = (s) => {
            const dir = ((s && s.addresses) || []).find((a) => /^\d+\.\d+\.\d+\.\d+$/.test(a));
            return dir || null;
        };
        const limpiarNombre = (n) => (n || '').replace(/\s*@.*$/, '').trim();
        const limpiarModelo = (m) => (m ? String(m).replace(/[()]/g, '').trim() : null);
        browsers.forEach((br) => {
            br.on('up', (s) => {
 ...
 */
function escanearImpresorasRed(connectTimeoutMs, locales) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const sondear = (ip, puerto, timeoutMs) => new Promise((resolve) => {
        ...
        const abiertosPorIp = new Map();
        const registrar = (ip, puerto) => {
            const s = abiertosPorIp.get(ip) || new Set();
            s.add(puerto);
            abiertosPorIp.set(ip, s);
        };
        // Pool de concurrencia sobre una lista de tareas.
        const correr = (tareas, concurrencia, timeoutMs) => __awaiter(this, void 0, void 0, function* () {
            let idx = 0;
            const worker = () => __awaiter(this, void 0, void 0, function* () {
                while (idx < tareas.length) {
                    const t = tareas[idx++];
                    // eslint-disable-next-line no-await-in-loop
                    if (yield sondear(t.ip, t.puerto, timeoutMs)) {
                        registrar(t.ip, t.puerto);
                    }
                }
            });
            yield Promise.all(Array.from({ length: Math.min(concurrencia, tareas.length || 1) }, worker));
        });
        ...
        yield correr(tareasVida, CONC_VIDA, connectTimeoutMs);
        ...
        abiertosPorIp.forEach((puertos, ip) => {
            [631, ...PUERTOS_IMPRESORA].forEach((puerto) => { if (!puertos.has(puerto)) {
                tareasDetalle.push({ ip, puerto });
            } });
        });
        if (tareasDetalle.length) {
            yield correr(tareasDetalle, CONC_RETRY, connectTimeoutMs + 500);
        }
        const mapa = new Map();
        yield Promise.all(Array.from(abiertosPorIp.keys()).map((ip) => __awaiter(this, void 0, void 0, function* () {
            const puertos = abiertosPorIp.get(ip) || new Set();
            ...
            try {
                ...
                const hosts = yield Promise.race([
                    dnsp.reverse(ip),
            ...
            catch ( /* sin PTR: se queda sin nombre y la UI muestra la IP */_a) { /* sin PTR: se queda sin nombre y la UI muestra la IP */ }
        })));
        return mapa;
    });
}
...
function registerPrinterIpcHandlers() {
    ...
    ipcMain.handle('detect-local-devices', () => __awaiter(this, void 0, void 0, function* () {
        try {
            return yield detectarDispositivosLocales();
        }
        catch (error) {
    ...
    ipcMain.handle('detect-network-printers', (_event, opts) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const [mdns, red] = yield Promise.all([
                descubrirImpresorasMdns(timeoutMs, locales),
                escanearRed ? escanearImpresorasRed(connectTimeoutMs, locales) : Promise.resolve(new Map()),
            ]);
            // Fusion por IP: mDNS aporta nombre/modelo; el barrido aporta puertos abiertos reales.
            const fusion = new Map(mdns);
            red.forEach((e, ip) => {
            ...
            return Array.from(fusion.values()).map((p) => {
                ...
                if (rpColaCups) {
                    uri = `ipp://${p.ip}:631/${rpColaCups}`;
                }
                else if (p.puertos.has(9100)) {
                    uri = `socket://${p.ip}:9100`;
                }
                else if (p.puertos.has(631)) {
                    uri = `ipp://${p.ip}:631/${p.rp || 'ipp/print'}`;
                }
                else if (p.puertos.has(515)) {
                    uri = `lpd://${p.ip}/queue`;
                }
                else {
                    uri = `socket://${p.ip}:9100`;
    ...
    ipcMain.handle('print-with-pos-printer', (event, args) => __awaiter(this, void 0, void 0, function* () {
        try {
                ...
                for (let i = 0; i < printData.length; i++) {
                    ...
                    if (item.type === 'image' && item.path && item.path.startsWith('data:image')) {
                        try {
                            ...
                            if (base64Data) {
                                fs.writeFileSync(imgPath, buffer_1.Buffer.from(base64Data, 'base64'));
                    ...
                    const buildEscPos = () => __awaiter(this, void 0, void 0, function* () {
                        ...
                        for (const item of printData) {
                            if (item.type === 'text') {
                                ...
                                const dataBuf = buffer_1.Buffer.from(val, 'ascii');
                                ...
                                try {
                                    const buf = buffer_1.Buffer.from(item.value, 'base64');
                                    push(buf);
                                }
                                catch (e) {
                    ...
                    return yield new Promise((resolve) => {
                        (0, child_process_1.exec)(cmd, (error, stdout, stderr) => {
                            ...
                            if (error) {
                                ...
                                resolve({ success: false, error: (error === null || error === void 0 ? void 0 : error.message) || 'EPL error' });
                                return;
                            }
            ...
            try {
                ...
                const errObj = printError;
                const msg = (errObj === null || errObj === void 0 ? void 0 : errObj.message) || (typeof printError === 'string' ? printError : JSON.stringify(printError)) || 'Unknown printing error';
                return {
                    success: false,
                    error: msg
                };
            ...
            const errObj = error;
            const msg = (errObj === null || errObj === void 0 ? void 0 : errObj.message) || (typeof error === 'string' ? error : JSON.stringify(error)) || 'Unknown printing error';
...
try {
    electron_1.app.on('ready', () => {
        ...
        electron_updater_1.autoUpdater.on('error', (err) => {
            ...
            if (win)
                win.webContents.send('update-status', `Error en actualizacion: ${err === null || err === void 0 ? void 0 : err.message}`);
        });
    });
...
function printImageWithCUPS(printer, content) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const imageBuffer = buffer_1.Buffer.from(base64Data, 'base64');
            ...
            return new Promise((resolve, reject) => {
                (0, child_process_1.exec)(printCommand, (error, stdout, stderr) => {
                    ...
                    if (error) {
                        console.error(`CUPS image printing error: ${error === null || error === void 0 ? void 0 : error.message}`);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (isImagePrint) {
            try {
                ...
                const imageBuffer = buffer_1.Buffer.from(imageData.replace(/^data:image\/\w+;base64,/, ''), 'base64');
                console.log(`Image buffer created, size: ${imageBuffer.length} bytes`);
                const imageTempFile = path.join(electron_1.app.getPath('temp'), `label-${Date.now()}.png`);
                fs.writeFileSync(imageTempFile, imageBuffer);
                console.log(`Image saved to temp file: ${imageTempFile}`);
            ...
            if (isRotatedLabel) {
                ...
                return new Promise((resolve, reject) => {
                    (0, child_process_1.exec)(printCommand, (error, stdout, stderr) => {
                        ...
                        if (error) {
                            console.error(`CUPS rotated label printing error: ${error === null || error === void 0 ? void 0 : error.message}`);
                            console.error(`stderr: ${stderr}`);
                            reject(error);
                            return;
                        }
                ...
                return new Promise((resolve, reject) => {
                    (0, child_process_1.exec)(printCommand, (error, stdout, stderr) => {
                        ...
                        if (error) {
                            console.error(`CUPS label printing error: ${error === null || error === void 0 ? void 0 : error.message}`);
                            console.error(`stderr: ${stderr}`);
                            reject(error);
                            return;
                        }
            ...
            return new Promise((resolve, reject) => {
                (0, child_process_1.exec)(printCommand, (error, stdout, stderr) => {
                    ...
                    if (error) {
                        console.error(`CUPS printing error: ${error === null || error === void 0 ? void 0 : error.message}`);
                        console.error(`stderr: ${stderr}`);
                        reject(error);
                        return;
                    }
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (content.includes('__IMAGE_PRINT_DATA__:')) {
                ...
                const imgBuffer = buffer_1.Buffer.from(imageData.replace(/^data:image\/\w+;base64,/, ''), 'base64');
