...
import { VehiculoSucursal } from '../../vehiculo/models/vehiculo-sucursal.model';
...
export class ListVehiculoSucursalComponent implements OnInit {
    ...
    cargarDatos(): void {
        ...
        // Si hay filtro por sucursal, usar el query específico (sin paginación por ahora)
        if (sucursalId) {
            this.vehiculoService.onBuscarVehiculosSucursalPorSucursal(sucursalId).subscribe({
                next: (res) => {
                    let datosFiltrados = res || [];
                    if (responsableId) {
                        datosFiltrados = datosFiltrados.filter(vs => vs.responsable?.id === responsableId);
                    }
                    this.dataSource.data = datosFiltrados;
                    this.totalElements = datosFiltrados.length;
            ...
            this.vehiculoService.onBuscarTodosVehiculosSucursal(this.pageIndex, this.pageSize).subscribe({
                next: (res) => {
                    let datosFiltrados = res || [];
                    if (responsableId) {
                        datosFiltrados = datosFiltrados.filter(vs => vs.responsable?.id === responsableId);
                    ...
                    this.dataSource.data = datosFiltrados;
                    // Como el backend no devuelve PageInfo, usamos la longitud de la respuesta
                    // Si tiene menos elementos que pageSize, es la última página
                    this.totalElements = datosFiltrados.length < this.pageSize 
                        ? (this.pageIndex * this.pageSize) + datosFiltrados.length
                        : (this.pageIndex + 1) * this.pageSize + 1; // Asumimos que hay más
                    this.isLoading = false;
                    this.cdr.markForCheck();
                },
                error: () => {
