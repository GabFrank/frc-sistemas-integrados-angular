...
graph TD
    subgraph "1. Planificación de Pedido"
        ...
        PID -->|N..1| SI[Sucursal Influencia]
        PID -->|N..1| SE[Sucursal Entrega]
    end
...
- **Propósito:** Planifica la entrega de una cantidad de un `PedidoItem` diferenciando entre la sucursal que necesita el producto (influencia) y donde será entregado físicamente (entrega). Esto permite manejar escenarios donde el producto se entrega en una sucursal central pero está destinado para otra sucursal.
- **Campos:**
    - `id`: `Long` - Identificador único.
    - `pedidoItem`: `PedidoItem` - El ítem de pedido que se está distribuyendo.
    - `sucursalInfluencia`: `Sucursal` - La sucursal para la cual se está realizando la compra (quien realmente necesita el producto).
    - `sucursalEntrega`: `Sucursal` - La sucursal donde físicamente se entregará el producto.
    - `cantidadAsignada`: `Double` - La cantidad de producto destinada a esa sucursal de influencia.
- **Relaciones:**
    - **`PedidoItem` (N-1):** Es parte de la distribución de un ítem de pedido.
    - **`Sucursal (Influencia)` (N-1):** Indica la sucursal que necesita el producto.
    - **`Sucursal (Entrega)` (N-1):** Indica la sucursal donde se entregará físicamente el producto.
- **Lógica de Negocio:**
    - Sucursal de influencia y entrega pueden ser la misma sucursal.
    - Si se selecciona una única sucursal de entrega, será utilizada para todas las sucursales de influencia.
    - Se crea un registro por cada sucursal de influencia que requiera el producto.
    - Los encargados de depósito pueden posteriormente redireccionar productos desde sucursal de entrega hacia sucursal de influencia.
