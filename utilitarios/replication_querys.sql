...
CREATE PUBLICATION publication_name FOR TABLE table1,
table2;
...
CREATE SUBSCRIPTION subscription_name CONNECTION 'host=cloud_server_ip dbname=cloud_db user=replication_user password=your_password' PUBLICATION publication_name;
...
ADD PUBLICATION another_publication_name;
...
SELECT subname,
    pid,
    received_lsn - replay_lsn AS replication_lag
FROM pg_stat_subscription;
-- script para crear subscription para el servidor
CREATE SUBSCRIPTION filial24_sub CONNECTION 'dbname=general host=172.25.1.24 user=franco password=franco port=5551' PUBLICATION filial24_pub WITH (copy_data = false, origin = 'none');
-- script para crear subscription para la filial
CREATE SUBSCRIPTION central_filial24_sub CONNECTION 'dbname=bodega host=172.25.1.200 user=franco password=franco port=5551' PUBLICATION central_filial24_pub WITH (copy_data = false, origin = 'none');
CREATE SUBSCRIPTION filial24_central_sub CONNECTION 'dbname=bodega host=172.25.1.200 user=franco password=franco port=5551' PUBLICATION central_pub WITH (copy_data = false, origin = 'none');
--end script
-- alterar una publicacion
ALTER PUBLICATION central_filial4_pub
ADD TABLE operaciones.stock_por_producto_sucursal
WHERE (sucursal_id = 24);
-- creat publicacion simples para filiales
CREATE PUBLICATION filial5_pub FOR TABLE administrativo.marcacion,
...
CREATE PUBLICATION central_filial5_pub FOR TABLE administrativo.marcacion
WHERE (sucursal_id = 5),
    configuraciones.inicio_sesion
WHERE (sucursal_id = 5),
    financiero.cambio_caja
...
SELECT *
FROM pg_create_logical_replication_slot('central_filial4_slot', 'pgoutput');
-- funciones para modificar la sequence de movimiento stock 
-- para filiales
DO $$
DECLARE last_id BIGINT;
new_start BIGINT;
BEGIN -- Get the last ID value from the table
SELECT COALESCE(MAX(id), 0) INTO last_id
FROM operaciones.movimiento_stock;
-- Debug output to verify the last_id
RAISE NOTICE 'Last ID from table: %',
last_id;
-- Determine if the last ID is odd or even
IF mod(last_id, 2) = 0 THEN -- It's even, so add 1 to make it odd
new_start := last_id;
ELSE -- It's already odd, so use it as the starting point
new_start := last_id + 1;
END IF;
-- Debug output to verify the new_start
RAISE NOTICE 'New start value for sequence: %',
new_start;
-- Set the sequence to the new value and set increment by 2
PERFORM setval(
    'operaciones.movimiento_stock_id_seq',
    new_start,
    true
);
