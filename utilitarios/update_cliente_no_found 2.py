...
def generate_sql(persona_id, cliente_id, nombre, documento, api_data):
    ...
    if api_data and api_data["procesamientoCorrecto"]:
        ...
        #     f"UPDATE personas.persona SET nombre='{nombre_api}', direccion='{direccion_api}' WHERE id={persona_id};"
        # )
        # sql_statements.append(
        #     f"UPDATE personas.cliente SET tributa=true, verificado_set=true WHERE id={cliente_id};"
        # )
    else:
        # Case where RUC is not found
        sql_statements.append(
            # f"UPDATE personas.cliente SET tributa=false, verificado_set=true WHERE id={cliente_id};"
            f"-- cliente no po encontrado. Nombre: '{nombre}, Id_persona: '{persona_id}', Id cliente: '{cliente_id}, documento: '{documento}'"
        )
